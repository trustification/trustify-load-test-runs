...
REALM_OPTS+=(-s registrationAllowed=false)
...
if [[ -n "$GITHUB_CLIENT_ID" ]]; then
  ID=$(kcadm get identity-provider/instances/github -r "${REALM}" --fields alias --format csv --noquotes || true)
  if [[ -n "$ID" ]]; then
...
if [[ -n "$ID" ]]; then
  kcadm update "users/$ID" -r "${REALM}" -s enabled=true -s email="nobody@example.com" -s firstName=Trustify -s lastName=Admin
else
  kcadm create users -r "${REALM}" -s "username=${CHICKEN_ADMIN}" -s enabled=true -s email="nobody@example.com" -s firstName=Trustify -s lastName=Admin
fi
